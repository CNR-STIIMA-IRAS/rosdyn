...
void IdentificationGui::generationDoneCb(const actionlib::SimpleClientGoalState& state,const rosdyn_identification_msgs::MetoTrjGenResultConstPtr& result)
{
  m_generate_trajectories_btn->setText("1) Generate Trajectory");
  updateTrajectoriesList();
...
void IdentificationGui::executionDoneCb(const actionlib::SimpleClientGoalState& state, const moveit_planning_helper::ExecuteTrajectoryFromParamResultConstPtr& result)
{
  m_simulate_trajectories_btn->setText("2) Simulate Trajectory");
  m_execute_trajectories_btn->setText("3) Execute Trajectory");
